...
struct ViewTab: Identifiable { //Has to conform to Identifiable in order to be used in the ForEach. We don't use Hashable and Codable like we did in the Landmarks project because we're not moving data between this structure and a data file/source.
    ...
    //This way, we now don't have to use the .id property in the closure in LandmarkList.Swift in order to specify each element dynamically. Before, the closure was "List(landmarks, id: \.id) { this_landmark in" and then it was "List(landmarks) { this_landmark in" before we stopped iterating with the list and instead used a ForEach within the List {} because this allowed us to have a toggle button within the list.
